...
const toFormState = (
  m: { model: string; provider: string; baseUrl?: string } | null | undefined,
): ModelFormState =>
  m
    ? {
        modelName: m.model || "",
...
const SettingsPage = () => {
  ...
  const [modelConfig, setModelConfig] = useState<ModelFormState>(
    toFormState(null),
  );
  const [lightweightConfig, setLightweightConfig] = useState<ModelFormState>(
    toFormState(null),
  );
  const [embeddingConfig, setEmbeddingConfig] = useState<ModelFormState>(
    toFormState(null),
  );
  ...
  useEffect(() => {
    if (setting) {
      ...
      setEmbeddingConfig(
        setting.embedding
          ? {
              modelName: setting.embedding.model,
              provider: setting.embedding.provider,
              baseUrl: setting.embedding.baseUrl || "",
            }
          : toFormState(null),
      );
    }
    ...
    async () => {
      await api.updateSetting({
        model: {
          ...
          baseUrl:
            modelConfig.provider === "other" ? modelConfig.baseUrl : undefined,
        },
        lightweightModel: lightweightConfig.modelName.trim()
          ? {
              model: lightweightConfig.modelName,
              provider: lightweightConfig.provider,
              baseUrl:
                lightweightConfig.provider === "other"
                  ? lightweightConfig.baseUrl
                  : undefined,
            }
          : undefined,
        embedding: {
  ...
  const labelCls = "text-[10px] font-semibold theme-text-muted uppercase ";
        ...
        <div>
          ...
          <Select
            ...
            onChange={(v) =>
              setState((s) => ({
                ...s,
                provider: typeof v === "string" ? v : v?.[0] || "",
              }))
            }
        ...
        {state.provider === "other" && (
          <div>
            ...
            <input
              ...
              value={state.baseUrl}
              onChange={(e) =>
                setState((s) => ({ ...s, baseUrl: e.target.value }))
              }
      ...
      <div className="h-full flex flex-col overflow-hidden">
          ...
          <div className="w-full max-w-2xl mx-auto pb-6">
            ...
            <div className="space-y-4">
              {renderModelSection(
                ...
                setting
                  ? {
                      configured: setting.model.apiKeyConfigured,
              ...
              {renderModelSection(
                ...
                setting?.lightweightModel
                  ? {
                      configured: setting.lightweightModel.apiKeyConfigured,
              ...
              {renderModelSection(
                ...
                setting?.embedding
                  ? {
                      configured: setting.embedding.apiKeyConfigured,
        ...
        <div className="shrink-0 border-t theme-border theme-surface backdrop-blur p-4 md:p-6">
          <div className="max-w-2xl mx-auto flex items-center justify-between">
            <div
              className={`flex items-center gap-2 theme-accent-text text-[10px] font-semibold transition-all duration-500 ${
                showSaveToast
